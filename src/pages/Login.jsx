...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    e.preventDefault();
    try {
      const res = await axios.post('[https://toko-online-backend-96jc.onrender.com/api/auth/login](https://toko-online-backend-96jc.onrender.com/api/auth/login)', credentials);
    ...
    <div className="form-card">
      ...
      <form onSubmit={handleLogin}>
        <div style={{ marginBottom: '1.5rem' }}>
            ...
            <input type="text" placeholder="Masukkan username Anda" style={{ width: '100%', padding: '1rem', borderRadius: '10px', border: '1px solid #cbd5e1', backgroundColor: '#f8fafc', fontSize: '1rem', color: '#000' }} onChange={(e) => setCredentials({...credentials, username: e.target.value})} />
        </div>

        <div style={{ marginBottom: '2rem' }}>
            <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: '700', color: '#1e293b' }}>Password</label>
            <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
              <input type={showPassword ? "text" : "password"} placeholder="Masukkan password Anda" style={{ width: '100%', padding: '1rem', paddingRight: '5.5rem', borderRadius: '10px', border: '1px solid #cbd5e1', backgroundColor: '#f8fafc', fontSize: '1rem', color: '#000' }} onChange={(e) => setCredentials({...credentials, password: e.target.value})} />
